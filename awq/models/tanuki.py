...
from awq.modules.fused.block import TanukiBlock
from awq.modules.fused.model import TanukiModel
from awq.modules.fused.moe import FusedSparseMoeTanukiBlock
from awq.utils.fused_utils import fuse_qkv, fuse_linears
from .custom.modeling_tanuki import (
    TanukiDecoderLayer as OldTanukiDecoderLayer,
    TanukiForCausalLM as OldTanukiForCausalLM,
)
from awq.modules.linear import WQLinear_GEMM
from awq.modules.fused.norm import FasterTransformerRMSNorm


class TanukiAWQForCausalLM(BaseAWQForCausalLM):
    layer_type = "TanukiDecoderLayer"
    max_seq_len_key = "max_position_embeddings"
    modules_to_not_convert = ["gate"]

    @staticmethod
    def fuse_layers(model: OldTanukiForCausalLM):
        fuser = TanukiFuser(model)
        fuser.fuse_transformer()

    @staticmethod
    def get_model_layers(model: OldTanukiForCausalLM):
    ...
    @staticmethod
    def move_embed(model: OldTanukiForCausalLM, device: str):
        model.model.embed_tokens = model.model.embed_tokens.to(device)

    @staticmethod
    def get_layers_for_scaling(
        module: OldTanukiDecoderLayer, input_feat, module_kwargs
    ):
        layers = []
...
class TanukiFuser:
    def __init__(self, model: OldTanukiForCausalLM):
        self.model = model

        self.tanuki_blocks: List[Tuple[str, OldTanukiDecoderLayer]] = [
            (name, module)
            for name, module in self.model.named_modules()
            if "TanukiDecoderLayer".lower() in module.__class__.__name__.lower()
        ]

    def fuse_transformer(self):
        blocks = []

        module: OldTanukiDecoderLayer
        for module in tqdm.tqdm(self.model.model.layers, desc="Fusing layers..."):
            ...
            if isinstance(sparse_moe.experts[0].w1, WQLinear_GEMM):
                ...
                sparse_moe = FusedSparseMoeTanukiBlock(
